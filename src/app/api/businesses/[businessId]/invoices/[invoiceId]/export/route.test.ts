 ...
 *
 * Recovered from the Export V1 branch and rewired to this branch's PDF
 * engine (`renderInvoicePdf`) + its `PDF_EXPORT` entitlement gate, which
 * applies to the `pdf` format only.
 */
...
vi.mock("@/server/entitlements/entitlementService", () => entitlementsSvc);
...
beforeEach(() => {
  ...
  entitlementsSvc.resolveEntitlements.mockResolvedValue({});
  entitlementsSvc.entitlementHasFeature.mockReturnValue(true);
});
...
describe("GET export route", () => {
  it("streams the PDF with download headers and no-store", async () => {
    ...
    expect(entitlementsSvc.entitlementHasFeature).toHaveBeenCalledWith(
      expect.anything(),
      "PDF_EXPORT",
    );
  });
  ...
  it("denies PDF without the PDF_EXPORT entitlement and renders nothing", async () => {
    const { GET } = await import("./route");
    entitlementsSvc.entitlementHasFeature.mockReturnValue(false);
  ...
  it("does not gate non-PDF formats on the PDF entitlement", async () => {
    const { GET } = await import("./route");
    entitlementsSvc.entitlementHasFeature.mockReturnValue(false);

    for (const format of ["png", "jpg", "xlsx"]) {
      const response = await GET(requestFor(format), PARAMS);
      expect(response.status).toBe(200);
    }
    expect(entitlementsSvc.resolveEntitlements).not.toHaveBeenCalled();
  });
});
