 ...
 * The PDF attached is generated by the same `renderInvoicePdf` every
 * other export uses — finalized rows send their snapshots.
 */
/**
 * Lazily loads the PDF engine for the no-`pdfBytes` fallback. Static import
 * would pull the engine's module graph (preview/auth chain) into every
 * importer — including unit tests that inject `pdfBytes` and the Gmail
 * OAuth routes — so the heavy module loads only when bytes are actually
 * generated here. (Production callers like `exportActions` always inject
 * `pdfBytes`, making this path cold in practice.)
 */
async function renderInvoicePdfFallback(model: InvoicePreviewModel): Promise<Uint8Array> {
  const { renderInvoicePdf } = await import("./pdfService");
...
export async function sendInvoicePdfViaGmail(
  ...
  const pdfBytes = deps.pdfBytes ?? (await renderInvoicePdfFallback(model));
