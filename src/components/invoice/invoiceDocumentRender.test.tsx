...
describe("live preview document — the real invoice, not a placeholder", () => {
  it("renders the A4 sheet with the business letterhead and brand color", () => {
    ...
    expect(html).toContain("--pv-header-bg:#0f766e");
    expect(html).toContain("--pv-header-fg:#ffffff");
    expect(html).toContain("--pv-footer-bg:#111827");
    expect(html).not.toContain("text-[color:var(--pv-brand)]");
  ...
  it("renders تومان when the preview currency is IRT", () => {
    const { html } = renderForm({}, { ...CONTEXT, currency: "IRT" });
    expect(html).toContain("۴۴۱,۴۵۰ تومان");
    expect(html).not.toContain("۴۴۱,۴۵۰ ریال");
  });
